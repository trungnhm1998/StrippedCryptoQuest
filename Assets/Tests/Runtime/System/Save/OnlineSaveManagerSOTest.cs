...
namespace CryptoQuest.Tests.Runtime.System.Save
{
    ...
    {        
        ...
        public async Task SaveGame_ShouldReturnTrue()
        {
                ...
                {
                    var result = await _saveManagerSO.SaveAsync(new SaveData());
        ...
        public async Task LoadSaveGame_ShouldReturnTrue()
        {
                ...
                if (ret && _saveManagerSO != null)
                {
                    ...
                    Assert.That(saveData.playerName, Is.EqualTo(SaveData.DEFAULT_PLAYER_NAME), "LoadSaveGame should return value that contains default player name.");
                }
                else
                {
                    Assert.Fail("User not logged in");
