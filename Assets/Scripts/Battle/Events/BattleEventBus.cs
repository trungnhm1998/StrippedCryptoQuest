...
namespace CryptoQuest.Battle.Events
{
    ...
    {
        private static readonly ITinyMessengerHub _messageHub = new TinyMessengerHub(new DefaultSubscriberErrorHandler());

        public static TinyMessageSubscriptionToken SubscribeEvent<T>(Action<T> listener)
            where T : BattleEvent, new()
        {
            return _messageHub.Subscribe<T>(listener);
        }
        ...
        public static void RaiseEvent<T>(T message) where T : BattleEvent, new()
        {
